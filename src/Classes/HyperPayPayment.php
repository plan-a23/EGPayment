...
use PlanA23\EGPayment\Exceptions\MissingPaymentInfoException;
...
class HyperPayPayment implements PaymentInterface
{
    ...
    {
        $this->hyperpay_url = config('PlanA23-payments.HYPERPAY_URL');
        $this->hyperpay_base_url = config('PlanA23-payments.HYPERPAY_BASE_URL');
        $this->hyperpay_token = config('PlanA23-payments.HYPERPAY_TOKEN');
        $this->currency = config('PlanA23-payments.HYPERPAY_CURRENCY');
        $this->hyperpay_credit_id = config('PlanA23-payments.HYPERPAY_CREDIT_ID');
        $this->hyperpay_mada_id = config('PlanA23-payments.HYPERPAY_MADA_ID');
        $this->hyperpay_apple_id = config('PlanA23-payments.HYPERPAY_APPLE_ID');
        $this->app_name = config('PlanA23-payments.APP_NAME');
    ...
    public function verify(Request $request)
    {
        ...
        if (in_array($final_result["result"]["code"], ["000.000.000", "000.100.110", "000.100.111", "000.100.112"])) {
            ...
            return [
                'success' => false,
                'payment_id'=>$request['id'],
                'message' => __('PlanA23::messages.PAYMENT_FAILED_WITH_CODE', ['CODE' => $final_result["result"]["code"]]),
                'process_data' => $final_result
            ];
        }
    }
    ...
    {
        return view('PlanA23::html.hyper_pay', ['model' => $this, 'brand' => $this->getBrand()])->render();
    }
